...
//    					+	'<h2 ng-show="selectedStep.wzHeadingTitle != \'\'">'
...
//    					+'</div>\n')
    ...
    a.put("wizard.html", '<div>\n'
    					...
    					+	'<div class="steps" ng-if="indicatorsPosition === \'bottom\'" ng-transclude></div>\n'
			    		+	'<ul class="steps-indicator steps-{{getEnabledSteps().length}}" ng-if="!hideIndicators">\n'
			    		+		'<li ng-class="{default: !step.completed && !step.selected, current: step.selected && !step.completed, done: step.completed && !step.selected, editing: step.selected && step.completed, borderDown: indicatorsPosition=== \'top\', borderUp: indicatorsPosition=== \'bottom\'}" ng-repeat="step in getEnabledSteps()">\n'
			    		+			'<div ng-click="goTo(step)" class="iconStep">'
			    		+				'<i class="fa {{step.icon || step.wzIcon}}"></i>'
			    		+			'</div>'
			    		+			'<a ng-click="goTo(step)">{{step.title || step.wzTitle}}</a>\n'
			    		+		'</li>\n'
			    		+	'</ul>\n'
