...
public class MonitoreoIncidenciasDAOImpl extends BaseDaoHibernate<OrdenServicioDTO> implements MonitoreoIncidenciasDAO {
	...
	public List<MonitoreoIncidenciasVO> getInfoSeguimientoGeneral(String fechaInicio, String fechaFin, List<Long> idsCentroInstalacion) {
		...
		+ "	WHERE "
		+ "		TIE051.ST_ACTIVO = 1 "
		+ "		AND TIE051.ID_CENTRO_INSTALACION = CI.ID_CENTRO_INSTALACION "
		+ "		AND ( "
		+ "			CASE "
		+ "				WHEN TIE026.ID_ORDEN_SERVICIO IS NOT NULL AND ( "
		+ "					TIE026.ST_ACTIVO = 1 "
		+ "					AND TRUNC(TIE026.FH_CITA) BETWEEN TO_DATE('#{fechaInicio}','dd/MM/yyyy') AND TO_DATE('#{fechaFin}','dd/MM/yyyy')) "
		+ "					THEN 1 "
	...
	public List<MonitoreoIncidenciasVO> getInfoSeguimientoXModulo(
		...
		+ "LEFT JOIN TIE035C_IE_PROCESOS TIE035 ON TIE035.ID_PROCESO = TIE026.ID_PROCESO_ACTUAL "
		+ "LEFT JOIN TIE001D_EE_ENCUESTAS TIE001 ON TIE001.ID_ENCUESTA = TIE026.ID_ENCUESTA_ACTUAL "
		+ "WHERE "
		+ "	TIE051.ST_ACTIVO = 1 "
